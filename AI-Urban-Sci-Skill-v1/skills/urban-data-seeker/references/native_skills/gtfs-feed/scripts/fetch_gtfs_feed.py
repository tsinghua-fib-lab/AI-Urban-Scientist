...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch a GTFS feed sample zip under policy control.")
    ...
    input_payload = {"fixture_zip": bool(args.fixture_zip), "url": args.url, "max_bytes": args.max_bytes, "download_mode": fetch_mode(args.policy)}
    ...
    if not policy_allows(args.policy, "fetch"):
        ...
        return
    try:
        target = Path(args.output_dir) / "gtfs_feed_sample.zip"
        artifact = fetch_file_artifact(
            fixture_file=args.fixture_zip,
            url=args.url,
            target=target,
            policy=args.policy,
