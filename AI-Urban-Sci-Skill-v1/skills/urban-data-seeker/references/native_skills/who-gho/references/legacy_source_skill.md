# WHO Global Health Observatory SourceSkill

## Purpose

Emit not-final source-prior hints for WHO GHO health indicator leads using the official OData API.

## When To Use

Use when the need asks for health/global indicators and this source family is a plausible publisher, catalog, repository, or provider.
...
- `scripts/find_who_gho.py` emits not-final source-card route hints.
Platform mechanics may be delegated to: `odata_platform`.
